...
import { IconoDeslizar, IconoPellizco, IconoDedo } from './SenalesManos'
...
export function Cinematica() {
   ...
   */
  useEffect(() => {
    if (listo || !(terminado && progress >= 100)) return
    const t = setTimeout(() => setListo(true), SOSTIENE)
    return () => clearTimeout(t)
  }, [terminado, progress, listo])

  /*
   * El tope de seguridad, aparte y no dentro del efecto de arriba.
   *
   * Metido ahí, con `progress` en las dependencias, se reiniciaba cada vez
   * que el porcentaje se movía —cosa que hace todo el rato mientras algo
   * pesado sigue bajando—, así que en la práctica nunca llegaba a cumplirse:
   * esperaba a que la descarga entera terminara, no a los 9 segundos que decía
   * el comentario. Puesto aquí, en un efecto que se monta una sola vez, es un
   * reloj de verdad: si algo falla o tarda demasiado, el botón sale igual.
   */
  useEffect(() => {
      ...
      */}
      {listo && (
        <div className="entra pointer-events-none absolute inset-x-0 bottom-28 z-20 flex flex-col items-center gap-5 px-6 text-center md:bottom-32">
          ...
          {/*
            La señalética del control por gestos, ya aquí en la portada.

            La frase de arriba dice que existe un control por manos; esto dice
            cómo se usa, antes de que nadie tenga que descubrirlo a tientas
            frente a la cámara. Tres gestos, no los siete del panel completo
            —ver ControlManos.tsx—: los justos para arrancar, no un manual.
          */}
          <div className="flex items-start gap-5 pt-1">
            {[
              { Icono: IconoDeslizar, texto: <>Mano derecha<br />arriba / abajo</> },
              { Icono: IconoPellizco, texto: <>Mano izquierda<br />pellizco = zoom</> },
              { Icono: IconoDedo, texto: <>Un dedo<br />= cursor</> },
            ].map(({ Icono, texto }, i) => (
              <div key={i} className="flex w-16 flex-col items-center gap-1 md:w-20">
                <Icono className="h-7 w-7 md:h-8 md:w-8" />
                <p className="text-center font-mono text-[0.52rem] uppercase leading-tight tracking-[0.08em] text-paper/45 md:text-[0.58rem]">
                  {texto}
                </p>
              </div>
            ))}
